...
def _create_table(
    ...
    coltipo = []
    for k, v in postgresql_types.items():
        col = k.replace(' ','').replace('-','_')
        tipo = v
        rows = f'{col} {tipo}'
        coltipo.append(rows)

    cols_str: str = "".join([f'{k},\n' for k in coltipo])[:-2]
...
def to_sql(
    ...
    try:
        with con.cursor() as cursor:
            ...
            #column_placeholders = '('+','.join([i for i in df.columns])+')'
            insertion_columns = ""
            upsert_str = ""
            if use_column_names:
            ...
            for placeholders, parameters in placeholder_parameter_pair_generator:
                ...
                l = list(map(lambda x: str(x).replace('None', ''), parameters))
                #values = ','.join(["'" + str(i) + "'" if i else 'NULL' for i in l])
                teste = [str(i) if i else None for i in l]
                #query = f'INSERT INTO "{schema}"."{table}" {column_placeholders} VALUES ({values})'
